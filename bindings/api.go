...
func GetAddressInfo(address string) ([]byte, error) {
	...
	for i, utxo := range utxos {
		tx, err := mempool.GetTransactionRaw(utxo.Txid)
		if err != nil {
			return marshalResponse(nil, fmt.Errorf("mempool.GetTransactionRaw: %v", err))
		}
		if int(utxo.Vout) >= len(tx.MsgTx().TxOut) {
			return marshalResponse(nil, fmt.Errorf("invalid wire.Tx retrieved for %s:%d", utxo.Txid, utxo.Vout))
		}

		pkScript := tx.MsgTx().TxOut[utxo.Vout].PkScript
...
func SignSlotSweepTransaction(requestBytes []byte) ([]byte, error) {
	...
	tx, err := satscard.SignTransaction(request.AddressInfo, request.Transaction, request.PrivateKey)
	if err != nil {
